...
pca_matrix <- trans_cts %>% 
  ...
  as.matrix() %>%  #as.matrix() converts our dataframe to a matrix
...
pc_eigenvalues <- tibble(PC = factor(1:length(pc_eigenvalues)),
  ...
  mutate(pct_cum = cumsum(pct)) #cum stands for cumulative
...
# Columns:
# Gene -> With the gene name.
# baseMean -> The normalized expression level of a gene
# log2FoldChange -> The amount of change between 2 conditions
# lfcSE -> Standard error associated to log2FoldChange value
# stat -> Statistics value computed as log2FoldChange / lfcSE and then compared to standard normal distribution
...
# MA plot
# We would generate a MA plot (baseMean vs log2FoldChange) and organize panels by comparison (timepoint)

test_results %>% 
  ggplot(aes(x = log(baseMean), y = log2FoldChange)) +
...
candidate_gene <- test_results %>% 
  filter(padj < 0.01) %>% 
  pull(gene) %>% # same as writing test_results[,"gene"] or test_results$gene
  unique()

# 1a. Get the trans_cts in long format
trans_cts <- read_csv("data_rnaseq/counts_transformed.csv")
